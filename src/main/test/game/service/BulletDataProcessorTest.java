    ...
    Object[][] bulletDataInput() {
        return new Object[][] {
                ...
                { GameConfig.STAGE_POS_LIMIT_X, 0.0d, 0.0d, 1,
                        GameConfig.STAGE_NEG_LIMIT_X + GameConfig.BULLET_INITIAL_SPEED, 0.0D },
                { GameConfig.STAGE_NEG_LIMIT_X, 0.0d, 180.0d, 1,
                        GameConfig.STAGE_POS_LIMIT_X - GameConfig.BULLET_INITIAL_SPEED, 0.0D }, };
    }

    @Test(dataProvider = "bulletDataInput")
    public void testShouldUpdateBulletCoordinates(double x, double y, double angle, int cycles, double expectedX,
            double expectedY) {
        // given
        ...
        Assert.assertEquals(bulletX, expectedX, 0.0001,
                "Expected and calculated X coordinates does not match! (expected: " + expectedX + ", calculated: "
                        + bulletX + ")");
        Assert.assertEquals(bulletY, expectedY, 0.0001,
