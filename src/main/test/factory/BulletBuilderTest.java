...
public class BulletBuilderTest {
    ...
    @Test(
            ...
            expectedExceptionsMessageRegExp = ".*angle.*"
    )
    ...
    @Test(
            expectedExceptions=RuntimeException.class, 
            expectedExceptionsMessageRegExp = ".*playerId.*"
            )
    public void testShouldThrowPlayerIdException() {
        BulletBuilder bulletBuilder = new BulletBuilder();
        Point2D coordinate = new Point2D.Double(0.0, 0.0);
        bulletBuilder.setCoordinate(coordinate)
        .setAngle(1.0)
        .build();
    }
    @Test(
            expectedExceptions=RuntimeException.class, 
            expectedExceptionsMessageRegExp = ".*damage.*"
            )
    ...
    public void testShouldBuildBullet() {
        BulletBuilder bulletBuilder = new BulletBuilder();
        Point2D coordinate = new Point2D.Double(0.0, 0.0);
        Bullet bullet = bulletBuilder.setCoordinate(coordinate)
        .setAngle(1.0)
        .setPlayerId(1)
        .setDamage(1)
        .build();
